...
function findIndexByToken(headerTokens: string[], candidates: string[]) {
  return headerTokens.findIndex((h) => candidates.includes(h));
}
...
export async function runLegendsIngest() {
  ...
  if (!res.ok) {
    return { ok: false as const, status: 502, error: "failed to fetch legends" };
  }
  ...
  $("table").each((_, table) => {
    ...
    const looksLikeLadder =
      headersLower.some((h) => h === "team") &&
      headersLower.some((h) => h.includes("pts") || h.includes("point"));
  ...
  if (!ladderHeaders.length || !ladderRowsRaw.length) {
    return {
      ok: false as const,
      status: 500,
      error: "Could not find ladder table containing Briars. Legends layout may have changed.",
    };
  }

  const h = ladderHeaders.map((x) => headerToken(String(x || "")));
  const idxPlayed = findIndexByToken(h, ["games", "game", "played", "gp"]);
  const idxWins = findIndexByToken(h, ["wins", "win", "w"]);
  const idxDraws = findIndexByToken(h, ["draws", "draw", "d"]);
  const idxLosses = findIndexByToken(h, ["losses", "loss", "l"]);
  const idxGF = findIndexByToken(h, ["gf", "g+", "goalsfor", "for", "f"]);
  const idxGA = findIndexByToken(h, ["ga", "g-", "goalsagainst", "against", "a"]);
  const idxGD = findIndexByToken(h, ["gd", "+/-", "diff", "difference"]);
  const idxPts = findIndexByToken(h, ["pts", "pt", "points", "point", "p"]);
